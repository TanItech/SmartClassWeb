...
def staff_apply_leave_save(request):
    if request.method!="POST":
        ...
        staff_obj=Staffs.objects.get(admin=request.user.id)
        try:
            leave_report=LeaveReportStaff(staff_id=staff_obj,leave_data=leave_date,leave_message=leave_msg,leave_status=0)
            leave_report.save()
...
def staff_feedback_save(request):
    if request.method!="POST":
        ...
        staff_obj=Staffs.objects.get(admin=request.user.id)
        try:
            feedback=FeedBackStaff(staff_id=staff_obj,feddback=feedback_msg,feddback_reply="")
            feedback.save()
...
def start_live_classroom(request):
    user=CustomUser.objects.get(id=request.user.id)
    staff=Staffs.objects.get(admin=user)
    return render(request, "base/lobby.html",{"user":user,"staff":staff})

def live_classroom(request):
