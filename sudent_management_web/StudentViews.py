...
from sudent_management_web.models import Students, Courses, Subjects, CustomUser, Attendance, AttendanceReport, LeaveReportStudent, FeedBackStudent, SessionYearModel
...
def student_apply_leave_save(request):
    if request.method!="POST":
        ...
        student_obj=Students.objects.get(admin=request.user.id)
        try:
            leave_report=LeaveReportStudent(student_id=student_obj,leave_data=leave_date,leave_message=leave_msg,leave_status=0)
            leave_report.save()
...
def student_feedback_save(request):
    if request.method!="POST":
        ...
        student_obj=Students.objects.get(admin=request.user.id)
        try:
            feedback=FeedBackStudent(student_id=student_obj,feddback=feedback_msg,feddback_reply="")
            feedback.save()
...
def join_live_classroom(request):
    user=CustomUser.objects.get(id=request.user.id)
    student=Students.objects.get(admin=user)
    return render(request, "base/student_lobby.html",{"user":user,"student":student})

def student_live_classroom(request):

    return render(request, "base/student_room.html")